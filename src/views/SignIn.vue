<template>
  <v-container fill-height class="justify-center" style="background-color: #233a5f" fluid>
    <v-card
      :loading="signingIn"
      class="col-lg-4 col-sm-5 col-xl-3 py-0"
      style="margin-bottom: 48px; background-color: #17263d; box-shadow: 0 0 10px #17263d"
      dark
    >
      <v-card-text class="text-center pt-6">
        <v-img src="https://cdn.vuetifyjs.com/docs/images/logos/vuetify-logo-light.svg" class="d-inline-block" max-width="48"></v-img>
        <div class="text-h5 mb-4" style="color: #fff; letter-spacing: 2px !important">Vuetify Admin</div>

        <v-alert text color="warning" dismissible close-icon="mdi-close" v-model="warningAlert">{{ indications.warning }}</v-alert>
        <v-alert text color="error" dismissible close-icon="mdi-close" v-model="errorAlert">{{ indications.error }}</v-alert>
        <v-form ref="signInForm" lazy-validation>
          <v-text-field v-model="username" :rules="[v => !!v || '账号不能为空！']" label="账号*" required></v-text-field>
          <v-text-field
      ...
      <v-card-actions class="px-4 pt-4 pb-6">
        <div class="d-flex align-center" style="font-size: 15px; color: #959da4">
    ...
    <v-toolbar
      style="background-color: #17263d; width: 100%; color: #959da4; letter-spacing: 2px; font-size: 12px; font-weight: 618"
      dense
      absolute
      bottom
      flat
      ...
      if (valid) {
        ...
        if (response.status === 0) {
          if (process.env.NODE_ENV === 'development') {
            ...
            sessionStorage.setItem('menus-actions', JSON.stringify({ menus, actions }));
          }
